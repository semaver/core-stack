...
- **BREAKING:** on finding a global `ClassTable` stamped with an incompatible
  `_protocol_version`, `ClassTableProvider` now **throws
...
- Carries forward the full symbol-keyed API surface first shipped in 2.2.0 (see the
  ...
  `ClassTable`, and `IClassTableRef._protocol_version` ([#102]).
- `@semaver/core` dev/peer range widened to `^3.0.0`.
...
- **Facade users: no changes.** If you only import `Reflector` (and its query API),
  upgrade with no code changes.
- **Low-level metadata access.** String keys are gone; use the exported symbol-keyed
  names:
  ```ts
  // before (≤ 2.1.x)
  const meta = (metadataClass as any).__metadata__;
  // after (≥ 3.0.0)
  import {MetadataClassNames} from "@semaver/reflector";
  const meta = metadataClass[MetadataClassNames.METADATA];
  ```
- **The global class table is now non-enumerable.** It no longer appears in
  `Object.keys(globalThis)`, `for..in`, object spread, or JSON serialization. Read it
  via the exported `ClassTableNames` symbol keys and `Reflect`, not by string name.
- **Protocol-version mismatch now throws.** If two incompatible copies of
  `@semaver/reflector` are loaded into one realm, construction now throws
  `ClassTableProtocolMismatchError` rather than warning. Deduplicate the dependency
  (single copy per realm) to resolve it.
...
> **Yanked — unpublished from npm.** The symbol-keyed API below was a breaking change
> mis-released as a minor version. Superseded by **[3.0.0]**, which ships the same API
> under a correct major bump. Do not depend on 2.2.0.
